...
class NTPClient:
    ...
    async def query_server(self, server: str) -> NTPResponse:
        ...
        try:
            ...
            raise NTPError(f"Timeout querying NTP server {server}")
        except socket.gaierror as e:
            raise NTPError(f"DNS resolution failed for {server}: {e}")
        except Exception as e:
            raise NTPError(f"Failed to query NTP server {server}: {e}")
